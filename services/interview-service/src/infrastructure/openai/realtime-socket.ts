import type { ProctoringSignal } from "@ai-interviewer/api-types";
import { env } from "../../config/env";
import { buildInterviewInstructions } from "../../domain/interview/interview-instructions";
import { buildProctoringAgentPrompt } from "../../domain/proctoring/proctoring.messages";
...
export class OpenAIRealtimeSocket {
  ...
  async connect() {
    ...
    const url = `wss://api.openai.com/v1/realtime?model=${encodeURIComponent(env.openaiRealtimeModel)}`;
  ...
  requestCandidateEndClosing(candidateName: string) {
    this.send({
      type: "response.create",
      response: {
        output_modalities: ["audio"],
        instructions: `${candidateName} asked to end the interview early. Acknowledge their request warmly, thank them for their time, give a brief positive closing remark, clearly say the session is ending now, wish them well, and say goodbye. Do not ask any questions.`,
      },
    });
  }
