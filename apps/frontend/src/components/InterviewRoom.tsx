...
export function InterviewRoom({
  ...
  return (
    <InterviewFlowShell showNavbar={false}>
      <div className="flex min-h-screen flex-col">
        <header className="flex items-center justify-between gap-4 border-b border-white/10 bg-background/40 px-4 py-3 backdrop-blur-xl sm:px-6">
          ...
          <div className="flex w-full max-w-5xl flex-col items-center gap-5">
            <div className="grid w-full max-w-3xl grid-cols-1 justify-items-center gap-5 sm:grid-cols-2">
              <ParticipantTile
                ...
                accent="agent"
              >
                <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden bg-gradient-to-br from-indigo-950 via-slate-950 to-blue-950">
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(99,102,241,0.25),transparent_45%)]" />
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_80%,rgba(59,130,246,0.18),transparent_40%)]" />
                  <div
                    className={cn(
                      "relative flex h-24 w-24 items-center justify-center rounded-full border-2 border-indigo-400/30 bg-indigo-500/10 shadow-[0_0_60px_rgba(99,102,241,0.25)] transition-transform duration-300 sm:h-28 sm:w-28",
                      agentSpeaking && "scale-105 border-indigo-300/60 shadow-[0_0_80px_rgba(99,102,241,0.45)]",
                    )}
                  >
                    <Bot className="h-10 w-10 text-indigo-200 sm:h-12 sm:w-12" />
                    {agentSpeaking && (
                      <span className="absolute inset-0 animate-ping rounded-full border border-indigo-300/40" />
                    )}
                  </div>
              ...
              >
                <div className="relative aspect-square w-full overflow-hidden bg-black/50">
                  ...
                  {userSpeaking && isMicEnabled && (
                    <div className="absolute bottom-4 left-4 rounded-full border border-indigo-400/30 bg-black/50 px-3 py-1 text-xs text-indigo-200 backdrop-blur-sm">
                      Speaking
                    </div>
                  )}
                </div>
            ...
            <aside className={cn("flex h-56 w-full max-w-3xl flex-col overflow-hidden rounded-2xl backdrop-blur-xl sm:h-64", interviewSurfaceClass)}>
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          message.role === "user" ? interviewUserBubbleClass : interviewAgentBubbleClass,
                        )}
                      >
        ...
        <div className="fixed inset-x-0 bottom-0 z-20 border-t border-white/10 bg-background/80 px-4 py-4 backdrop-blur-xl">
  ...
  return (
    <div
      className={cn(
        ...
        active
          ? accent === "agent"
            ? "border-indigo-400/50 shadow-indigo-950/30"
            : "border-blue-400/40 shadow-blue-950/20"
          : "border-white/10",
      )}
    >
      <div className="relative w-full">{children}</div>
      <div className="flex items-center justify-between gap-3 border-t border-white/10 bg-background/50 px-4 py-3">
        <div>
        ...
        <span
          className={cn(
            ...
            active ? "bg-indigo-400 shadow-[0_0_12px_rgba(99,102,241,0.8)]" : "bg-muted-foreground/40",
          )}
        />
      </div>
    </div>
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        active ? cn(interviewOutlineButtonClass, "text-foreground") : cn(interviewOutlineButtonClass, "text-muted-foreground"),
      )}
    >
      {children}
    </Button>
...
function ProctoringBadge({ proctoring }: { proctoring: ProctoringState }) {
  ...
  if (proctoring.status === "initializing") {
    return (
      <span className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-muted-foreground sm:inline-flex">
  ...
  return (
    <span className="hidden items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-3 py-1.5 text-xs text-indigo-300 sm:inline-flex">
      <Eye className="h-3.5 w-3.5" />
      Proctored
    </span>
  );
...
function ConnectionBadge({ status }: { status: ConnectionStatus }) {
  ...
  if (status === "connected") {
    return (
      <span className="inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-3 py-1.5 text-xs text-indigo-300">
  ...
  return (
    <div className={cn("flex items-end gap-1", className)}>
      {[0, 1, 2, 3, 4].map((index) => (
        <span
          key={index}
          className="w-1.5 rounded-full bg-indigo-300/80"
          style={{
