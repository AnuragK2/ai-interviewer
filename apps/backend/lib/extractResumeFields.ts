...
const SECTION_PATTERNS: { key: keyof Pick<ParsedResume, "summary" | "skills" | "experience" | "education" | "projects">; labels: string[] }[] = [
  { key: "summary", labels: ["summary", "profile", "objective", "about"] },
  { key: "experience", labels: ["experience", "work experience", "employment", "professional experience", "work history"] },
...
export function extractResumeFields(rawText: string): ParsedResume {
  ...
  const name = preamble.find(line => !extractEmail(line) && !extractPhone(line) && line.length > 1);
